...
The data comes from a Kaggle dataset and is split into two separate csv files: one for accepted applicants, and one for rejected applicants @geor19. The accepted file contains over two million rows and one hundred and fifty-one columns. The rejected data contains over twenty-seven million rows and nine columns. The Kaggle posting does not efficiently provide a data dictionary for every column; however, another post in Kaggle provides information on each column @chan18. As stated in the introduction, the data was pulled from LendingClub. The author of the Kaggle dataset  LendingClub no longer has the option to download this data.
